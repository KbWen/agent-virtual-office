...
Entries rotated out of `.agentcortex/context/current_state.md` when the
`## Ship History` section exceeded `document_lifecycle.ship_history_max_entries`
(10), per `.agent/workflows/ship.md` §State Update. Newest-first, verbatim.
Relative links that pointed out of `.agentcortex/context/` were flattened to
plain paths so they do not resolve one level too shallow from this depth.
