...
> 桌子座標的 SSoT 是 `src/components/PixelOffice.jsx` 的 `DESK_DATA`（共 7 張）。`gate`（門神）**沒有桌子** —— 它的位置來自 `movementSystem.js` 的 `WAYPOINTS.gate = { x: 100, y: 80 }`，站在門口走廊。上圖原本漏掉 `designer` 與 `gate` 兩個位置。
