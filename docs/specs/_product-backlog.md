...
| AVO-124 | Agent appearance customization | product | brand | P3 | — | feature | Pending | NOTE (2026-06-14): name+color override already EXISTS+now documented (`?agents=` / `window.__office_config__`, README). AVO-124 = the SPRITE-cosmetic layer (hats/accessories/outfits). Two impl paths: (a) procedural — let users override CHAR_STYLES (hairStyle/clothes) per role, no asset pipeline needed; (b) PNG sprite-assets via AVO-160. characters.json has an `accessory` field that the renderer does NOT consume (CHAR_STYLES is authoritative) — reconcile when building. |
| AVO-160 | Custom sprite-asset pipeline (public/sprites/ PNG auto-load → replace procedural SVG) | product | brand | P3 | docs/SPRITE_REQUIREMENTS.md | feature | Pending | the unbuilt engine spec'd in SPRITE_REQUIREMENTS.md: detect `public/sprites/<id>.png`, switch from procedural getBaseSprite to image render, fallback on miss. Foundation for hand-drawn art + AVO-124(b). Currently 0% built (no loader, no public/sprites/). Doc corrected 2026-06-14 to mark it planned. |
| AVO-125 | Cozy micro-interactions | product | game-feel | P3 | docs/specs/cozy-micro-interactions.md | feature | Shipped | chill-fun wave; spec status:shipped, listed [Shipped] in SSoT Spec Index (backlog row was stale-Pending — fixed 2026-06-14 audit) |
...
| AVO-158 | Poke / acknowledge micro-interaction (in-place reaction, honest) | product | game-feel | P3 | docs/specs/poke-acknowledge.md | feature | Shipped | #158; replaces AVO-142 per ADR-005. INTERACTION = Model A (layered, NO new gesture; 3-lens gesture panel unanimous): click = inspector + in-place acknowledge (face-camera + single bob ~200ms + real-state bubble ~1.2s); re-click while inspector open = reaction-only (seeded quip pool/status, escalate 3rd→longer, 5th→cozy turn-away, reset 10s idle); keyboard Enter=both / Space=re-poke; prefers-reduced-motion=static bubble only; bubble role=status aria-live polite, reaction aria-hidden; right-click(mouse)=reaction+preventDefault, touch=reaction on every tap (no long-press). ZERO position/state write. Identity "make it mine" → AVO-124 |
| AVO-159 | Render-path perf: bubble-visibility memo + order-signature on position.y + GateWaitingTray narrow | chore | tech-debt | P2 | — | feature | Done | 2026-06-14 perf audit (pre-existing, NOT a poke/card regression). ✅(1) `selectVisibleBubbles` single-entry MEMO keyed on (agents ref, externalStatus ref, cap, rotation-epoch): N computes/write→1, behavior-identical (+10 bubbleVisibility tests). ✅(2) `getAgentOrderSignature`+`sortByY` key on `position.y` (snaps on arrival; both consistent). ✅(3) `GateWaitingTray` subscribes to a primitive awaiting-approval+workflow signature (bails on position ticks; behavior-identical). `hasActiveHelper` left as-is (primitive bool selector, bails on stable value — negligible). All headless visual-confirmed (bubble cap ≤3, z-order + render clean), 2013 tests green. |
...
| AVO-155 | Add same-pick guarantee test for socialTargetOverride | review-finding | social-chat | P2 | — | quick-win | Done | #131; tests/socialTargetOverride.test.js — "same-pick guarantee" (pre-seeded override → destination orbits that peer); de-flaked #148, BOM-stripped #147 |
...
| #122 | normalizePost.mjs runtime mirror | chore | tech-debt | P2 | docs/specs/status-field-schema-unification.md | feature | Done | shipped 2026-06-14 (PR #163) — collapsed to `statusContract.mjs` single source (node-safe .mjs); 3-lens expert review READY; bare-node + live server verified |
...
| #126 | Semgrep baseline and fail-on-new serious findings | infra | tech-debt | P1 | docs/reviews/2026-06-11-semgrep-baseline.md | feature | Done | #136; security.yml two-pass — full scan reports (baseline triaged-accepted), ERROR-severity scan BLOCKS (`--error`) so any NEW serious finding fails CI |
