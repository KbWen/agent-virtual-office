...
| AVO-103 | **Tool-call inventory bubble** — Office classifies by W3C verbs and MCP but doesn't show *which* tools are active. Small floating toolbelt strip above each agent. | P1 | Pending | Per-agent ext.task already known; render an icon strip (Bash/Read/Edit/MCP server icons). |
