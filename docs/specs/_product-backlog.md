...
| AVO-193 | A coffee-machine click on a fully-busy office gives no feedback at all | review-finding | game-feel | P3 | — | quick-win | Shipped | Filed out of AVO-191 rather than folded into it. With no honest cast, `triggerInteractiveEvent` falls through to `fireInteractionReaction`, which maps only `deploy-success`→ops and `eureka`→arch — so a `tea-break` click produces nothing. Strictly better than the relocation it replaces and nothing false is claimed (ADR-008 honest-neutral bar), but the click reads as broken. Closing it means a new reactor mapping, a locale key in en + zh-TW, and a design call about **who** reacts — content, not a bug fix. **Shipped 2026-09-20 (PR #241).** The machine answers for itself: screen `CAFE` -> `BUSY` plus three wisps of steam for 2s, chosen by the owner from rendered candidates. The refusal is untouched -- the feedback hangs off the falsy return of `triggerInteractiveEvent`, so nobody is moved and nothing is claimed; measured in a real browser, positions, statuses and the full id->bubble-text map are identical before and after. Steam is a CSS `@keyframes`, not SMIL (an `<animate begin="0s">` mounted after load renders already-finished). |
| AVO-194 | `lunch-nap` shows a genuinely-working agent as asleep — the second R1 hole, and it does not go through `pickParticipants` | review-finding | office-runtime | P2 | — | quick-win |  Shipped |  **Shipped 2026-09-02 (PR #220, squash `7cf428d`).** It was THREE sites, not the one this row recorded. Root cause was reachability rather than forgetfulness: the availability predicate was a closure INSIDE `pickParticipants`, so no time-linked handler could apply it even in principle. Now a module-scope `isAgentAvailable(id, agents, externalStatus)` shared by all three callers. (1) 12:00 lunch nap now draws from available agents only, and `setActiveEvent` moved BELOW the cast so an empty cast cannot arm the global event mutex for 45s with nobody asleep. (2) 14:00 post-lunch drowsiness was an unrecorded THIRD site with two harms — a fabricated `tired` expression AND a `null` bubble argument that CLEARS what the agent was saying; the ring and name-pill come from `externalStatus`, which bounds that to voice rather than state. The open product question in this row is answered: **one free agent IS a real cast** (`>0`, not `>=2`) — unlike `all` / `random-2-3` group fictions, a nap is per-agent and one person napping alone is honest, so the guard narrows the cast without disabling the beat. The pre-existing `Fix 1` test turned out to be asserting a PHANTOM lunch-nap (the old code armed `activeEvent` before computing the cast, and its Math.random accounting was wrong), and was corrected. 4 new tests, all mutation-verified; both sites additionally proven in a real headless browser.
| AVO-195 | `sim-soak` has no invariant for a STALE BEHAVIOUR LABEL — the office can say an agent is doing X for minutes while it is doing something else, and the gate cannot see it | review-finding | ci-infra | P2 | — | quick-win |  Shipped |  **Detection half shipped 2026-09-02.** `detectStaleLabels` lives in `soakInvariants.mjs` (the gate owns it; `officeRhythm` imports it, not the reverse) and surfaces as a NON-FAILING `warnings.staleLabel`, following the `groupStack` warn-then-promote precedent. Two designs were rejected on evidence first. A 90s threshold — set from control runs whose worst case was 74-78s — **false-positived on the very first real soak** at 94.9s and 100.5s. Restricting to event-set behaviours was **refuted by measurement**: `eat-snack` / `nap` / `stretch` / `chat` are all in the `doSchedule` pools, and of the twelve behaviours the event handlers set only `meeting` is event-only. The shipped threshold is derived from the ceiling instead — a behaviour lasts <=65s plus ~10-20s of walk, and `pickBehavior` CAN repeat because the anti-repeat ring guards messages not behaviours, so **180s** requires three consecutive identical picks to fire legitimately. `maxStaleLabelMs` prints every run regardless, because a binary verdict hides the trend: healthy `main` reads 57-99s against the 254s that motivated the check. Known limitation, documented in the module: an unchanged label cannot be distinguished from repeated identical picks, so this is a smoke signal rather than proof. Promotion to a failing violation remains open and should follow the same quiet-period route. |
| AVO-196 | Panel mode: a speaker just BELOW the crop (lounge / research) still shows its bubble inside the crop with no visible speaker | review-finding | ui-rendering | P3 | — | quick-win | Shipped | Found in the 2026-09-19 review round 2 (PR #236); pre-existing on `main`. Examples: lounge `stretch` at (180,490) puts its bubble at y 388–414, fully inside the wide and initial crops, speaker invisible; `coffee` at (80,475) likewise. PR #236 stopped the ABOVE / LEFT / RIGHT cases (an off-crop speaker's bubble is neither flipped nor clamped into view) and deliberately left this one: hiding the bubble collides with ADR-007 (voice is not suppressed), so it is a design decision, not a bug fix. Measure first with a staged panel capture (the round-2 reviewer's numbers are computed, not rendered). **Shipped 2026-09-20 (PR #240).** Owner chose the rule from rendered candidates: hide the bubble when the speaker's ANCHOR is outside the visible crop, except `blocked`/`awaiting-approval` (ADR-007 D1 licenses blocked to seize the bubble, and the panel control bar surfaces it anyway). The anchor is the same test the #236 flip and edge clamp use on the other three sides; a first cut allowed a head poking in above the bottom edge, which made the coffee spot (80,475) count as visible by 4 units. Correction to this row's earlier note: ADR-007 does NOT say voice is never suppressed. |
| AVO-197 | Every ONE-SHOT SMIL animation in the office is dead — a late-mounted `<animate>` snaps to its end value instead of playing | review-finding | game-feel | **P2** | — | feature | Shipped | Independent of AVO-193, which sidestepped this by using CSS. **Measured 2026-09-20** (headless Chromium, controlled experiment with a known-playing control): a one-shot SMIL element inserted after page load never animates — SMIL resolves `begin="0s"` against the DOCUMENT timeline, which is already in the past. AVO-135's done flash was mounted for **60 frames and visible on exactly 1** (opacity 0.393 on the mount frame, then 0.000 x59; radii 22.58 -> 30.00, i.e. a snap, not a ramp) — the celebratory flash is effectively invisible. Same defect, lower severity, at AVO-158 poke bob and AVO-136 desk-slam jitter (both end at `0 0`, so NO motion ever happens) and AVO-134 / AVO-110 entry pops (snap to the correct end state, so only the pop is lost). `repeatCount="indefinite"` animations are UNAFFECTED (always mid-cycle) and so is `<animateTransform additive="sum">` only insofar as it is also one-shot. Two fixes work: a CSS `@keyframes` (what AVO-193 shipped) or `begin="indefinite"` + `beginElement()` — both verified moving. Tier is `feature`, not quick-win: 5 shipped features' feel changes at once, so each needs owner sight before it lands. **Shipped 2026-09-20 (PR #244).** Fixed with `begin="indefinite"` + `beginElement()` on mount, NOT a CSS rewrite: AVO-136/158 are `additive="sum"` transforms on a root `<g>` already carrying `scale(CHAR_SCALE)`, where CSS `transform` replaces the base transform and CSS `translate` applies outside it, silently changing the bob distance. No duration, curve or `values` list was touched. All six verified in a real browser, same instrument both sides: DEAD (1-2 distinct values) -> PLAYS (19-43); the done flash went from 1 visible frame to 39. Shipped as `quick-win`, not the `feature` estimated here — the code is mechanical and the design was already approved per feature; the owner-sight requirement was kept as its own gate (frame strips approved before merge). |
