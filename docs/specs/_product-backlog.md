...
| AVO-196 | Panel mode: a speaker just BELOW the crop (lounge / research) still shows its bubble inside the crop with no visible speaker | review-finding | ui-rendering | P3 | — | quick-win | In Progress | Found in the 2026-09-19 review round 2 (PR #236); pre-existing on `main`. Examples: lounge `stretch` at (180,490) puts its bubble at y 388–414, fully inside the wide and initial crops, speaker invisible; `coffee` at (80,475) likewise. PR #236 stopped the ABOVE / LEFT / RIGHT cases (an off-crop speaker's bubble is neither flipped nor clamped into view) and deliberately left this one: hiding the bubble collides with ADR-007 (voice is not suppressed), so it is a design decision, not a bug fix. Measure first with a staged panel capture (the round-2 reviewer's numbers are computed, not rendered). |
