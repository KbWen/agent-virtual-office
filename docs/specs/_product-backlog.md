...
| AVO-159 | Render-path perf: hoist bubble-visibility out of per-agent selector + signature on position.y + GateWaitingTray status-narrow | chore | tech-debt | P2 | — | feature | Pending | 2026-06-14 perf audit (verified, pre-existing — NOT a poke/card regression): (1) `AgentCharacter.jsx:682` calls `selectVisibleBubbles` + `Date.now()` INSIDE a per-agent zustand selector → full sort+`new Set()` per agent per ~30fps store write — hoist to ONE store-derived `visibleBubbles:Set` slice; (2) `PixelOffice.jsx:714 getAgentOrderSignature` reads `targetPosition.y` (changes per waypoint) → re-renders PixelOffice mid-walk; use `position.y` (snaps on arrival only); (3) `GateWaitingTray` + `hasActiveHelper` subscribe to whole `agents` → narrow to status/derived scalar. **Behavior-sensitive render-path refactor — needs owner visual-confirm (memory: don't claim visual works from code alone).** AVO-156-style ellipse fix done in bubbleVisibility rankMap (this wave). |
