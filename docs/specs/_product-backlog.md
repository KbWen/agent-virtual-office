...
| AVO-159 | Render-path perf: bubble-visibility memo (done) + signature on position.y + GateWaitingTray status-narrow | chore | tech-debt | P2 | — | feature | In Progress | 2026-06-14 perf audit (verified, pre-existing — NOT a poke/card regression). ✅ **(1) DONE** — the biggest cost (`selectVisibleBubbles` full sort+`new Set()` run per-agent per ~30fps store write) fixed via a single-entry MEMO keyed on (agents ref, externalStatus ref, cap, rotation-epoch): N computes/write → 1, BEHAVIOR-IDENTICAL (chose memo over a store-slice hoist to avoid render-timing risk; +10 tests bubbleVisibility.test.js; headless visual-confirmed). REMAINING (smaller, optional): (2) `PixelOffice.jsx getAgentOrderSignature` `targetPosition.y`→`position.y` — behavior-sensitive (mid-walk paint order), defer; (3) `GateWaitingTray`/`hasActiveHelper` subscribe to whole `agents` → narrow. |
