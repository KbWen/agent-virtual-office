...
- **AC-12** Data-path wiring (the field must survive end to end): `reasonCode` is added to (a) the hook `newAgents` record literal (`office-status-hook.js:868-877`), (b) the hook existing-agents **merge-read** field list (`:829-835`) so a still-blocked agent carries it forward, (c) the SSE/poll transport `u` payload, and (d) the store `ext` rebuild literal as `ext[id].reasonCode = u.reasonCode || null` (`store.js:780-791`). The renderer reads `ext.reasonCode`, NOT the raw agent record.
