...
IF the active Work Log contains a `Recommended Skills` entry AND those skills list `/handoff` in their `phases:` metadata:
...
3. Explicitly state: "Applying [skill-name] strategy for handoff."
4. Apply each skill's handoff-specific expectations as additional summary requirements.
